...
func NewServer(password_s string, listenAddr string) (*Server, error) {
	bsPassword, err := password.ParsePassword(password_s)
	if err != nil {
		return nil, err
	}
...
func (server *Server) Listen(didListen func(listenAddr *net.TCPAddr)) error {
	return transport.ListenSecureTCP(server.ListenAddr, server.Cipher, server.handleConn, didListen)
}

// 解 SOCKS5 协议
// https://www.ietf.org/rfc/rfc1928.txt
func (server *Server) handleConn(localConn *transport.SecureTCPConnection) {
